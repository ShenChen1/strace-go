...
func TestNewTraceCommandOverridesArgv0WithoutChangingExecutable(t *testing.T) {
	...
	if cmd.Path != "/bin/true" {
		t.Fatalf("command path = %q, want /bin/true", cmd.Path)
	}
	if len(cmd.Args) != 2 || cmd.Args[0] != "sample" || cmd.Args[1] != "argument" {
		t.Fatalf("command args = %#v, want [sample argument]", cmd.Args)
	}
}
