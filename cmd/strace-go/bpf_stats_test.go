...
func TestSumBPFStatsValuesAggregatesAllCounters(t *testing.T) {
	values := []bpfBpfStats{
		{RingbufReserveFail: 1, RingbufCopyFail: 2, PayloadTruncatedEvents: 3, PendingUpdateFail: 4, OrphanExit: 5, PendingMismatch: 6},
		{RingbufReserveFail: 10, RingbufCopyFail: 20, PayloadTruncatedEvents: 30, PendingUpdateFail: 40, OrphanExit: 50, PendingMismatch: 60},
	}
	...
	if stats.RingbufReserveFail != 11 || stats.RingbufCopyFail != 22 ||
		stats.PayloadTruncatedEvents != 33 || stats.PendingUpdateFail != 44 || stats.OrphanExit != 55 || stats.PendingMismatch != 66 {
		t.Fatalf("sum stats = %+v, want 11/22/33/44/55/66", stats)
	}
}

func TestSumBPFStatsValuesEmptyReturnsAvailableZero(t *testing.T) {
	stats := sumBPFStatsValues(nil)
	if !stats.Available || stats.RingbufReserveFail != 0 || stats.PendingUpdateFail != 0 || stats.OrphanExit != 0 || stats.PendingMismatch != 0 {
