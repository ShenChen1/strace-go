...
func TestSyscallEventContextUsesTimeSetterTLVSections(t *testing.T) {
	t.Run("clock_settime timespec", func(t *testing.T) {
		...
		enterRaw := miscStructTLVEvent(t, "clock_settime", bpfEventTypeEnter, args, 0, enterPayload)
		enterRaw.EventFlags |= bpfEventFlagGenericEnter
		session.traceState().handleEnvelope(newTraceEventEnvelopeFromBPF(enterRaw))

		exitRaw := miscStructTLVEvent(t, "clock_settime", bpfEventTypeExit, args, -22, nil)
	...
	t.Run("settimeofday timeval and timezone", func(t *testing.T) {
		...
		enterRaw := miscStructTLVEvent(t, "settimeofday", bpfEventTypeEnter, args, 0, enterPayload)
		enterRaw.EventFlags |= bpfEventFlagGenericEnter
		session.traceState().handleEnvelope(newTraceEventEnvelopeFromBPF(enterRaw))

		exitRaw := miscStructTLVEvent(t, "settimeofday", bpfEventTypeExit, args, -22, nil)
		exitUpdate := session.traceState().handleEnvelope(newTraceEventEnvelopeFromBPF(exitRaw))
		ev := newSyscallEventContextFromView(session, exitUpdate.syscallView, 101, exitUpdate.pendingEnter, exitUpdate.payloadSections)
