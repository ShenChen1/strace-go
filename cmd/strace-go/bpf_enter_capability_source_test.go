...
func TestBPFEnterCapabilityTranslationUnitsOwnDispatchHeaders(t *testing.T) {
	...
	want := map[string][]string{
		...
		"handlers_enter_control.c":    {"#define STRACE_GO_ENTER_CONTROL 1", `#include "enter_dispatch.h"`},
...
func TestBPFEnterCapabilitySourcesDoNotIncludeForeignDispatchHeaders(t *testing.T) {
	...
	foreign := map[string][]string{
		"handlers_enter_generic.c":    {`#include "enter_fragment_dispatch.h"`, `#include "mmsg_enter_dispatch.h"`, `#include "mount_path_dispatch.h"`, `#include "quota_dispatch.h"`},
		"handlers_enter_payload.c":    {`#include "enter_fragment_dispatch.h"`, `#include "mmsg_enter_dispatch.h"`, `#include "mount_path_dispatch.h"`, `#include "quota_dispatch.h"`},
		"handlers_enter_path.c":       {`#include "enter_fragment_dispatch.h"`, `#include "mmsg_enter_dispatch.h"`, `#include "quota_dispatch.h"`},
		"handlers_enter_memory.c":     {`#include "mount_path_dispatch.h"`, `#include "quota_dispatch.h"`},
		...
		"handlers_enter_structured.c": {`#include "enter_fragment_dispatch.h"`, `#include "mmsg_enter_dispatch.h"`, `#include "mount_path_dispatch.h"`},
	}
