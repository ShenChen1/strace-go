...
func (h *StatxHandler) Handle(ctx *Context) Result {
	...
	res.ArgParts = append(res.ArgParts, formatStatxFlags(ctx, ctx.Args[2]))
	res.ArgParts = append(res.ArgParts, decodeFlags(ctx, ctx.Args[3], "statx_masks"))
	res.ArgParts = append(res.ArgParts, decodeStatxSnapshot(ctx, ctx.Args[4]))
	return res
}

func formatStatxFlags(ctx *Context, value uint64) string {
	value = uint64(uint32(value))
	if xlatFormat(ctx) == "raw" {
		return statxRawValue(value)
	}

	named := statxNamedFlags(ctx, value)
...
func statxNamedFlags(ctx *Context, value uint64) string {
	parts := []string{statxNamedFlagGroup(ctx, value&statxSyncTypeMask, "at_statx_sync_types")}
	if other := value &^ statxSyncTypeMask; other != 0 {
		parts = append(parts, statxNamedFlagGroup(ctx, other, "at_flags"))
	}
	return strings.Join(parts, "|")
}

func statxNamedFlagGroup(ctx *Context, value uint64, tableName string) string {
