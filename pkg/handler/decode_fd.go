...
func formatFDTarget(ctx *Context, fd int32, target string) string {
	...
	if path, details, ok := decodePIDFDTarget(target); ok {
		if ctx.Opts.ShowFDPIDFDValue() && details != "" {
			return fmt.Sprintf("%d<%s>", fd, details)
		}
		if ctx.Opts.ShowFDPathValue() {
			return fmt.Sprintf("%d<%s>", fd, path)
	...
	if path, details, ok := decodeSignalFDTarget(target); ok {
		if ctx.Opts.ShowFDSignalFDValue() && details != "" {
			return fmt.Sprintf("%d<%s>", fd, details)
		}
		if ctx.Opts.ShowFDPathValue() {
			return fmt.Sprintf("%d<%s>", fd, path)
