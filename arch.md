...
- `fstat` 已作为第一条 stat 类 OUT struct syscall 绕开旧 fixed-window capture；enter 阶段只保存小 pending metadata，exit 成功时直接写 144 字节 `struct stat` OUT TLV section。
