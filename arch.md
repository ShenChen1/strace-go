...
- `quotactl`/`quotactl_fd` 的 tracepoint metadata 已恢复真实 `id` 参数，但 native `quotactl.gen.test` 在旧 fallback 表和新 tracepoint 表下都仍依赖未实现的 quota 专用 xlat、path/struct payload 与 formatter；它暂不属于 curated reference，后续应按 direct TLV + syscall-specific handler 补齐，不能把 generic pointer 输出当成完成。
