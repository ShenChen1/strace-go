...
- `io_pgetevents` 仍由 `payloads` 描述 timeout/sigset/sigmask 和 event 数组；`io_getevents` 已从 capture policy 迁出，由 AIO direct TLV helper 显式捕获 timeout IN 和 events OUT sections。`io_submit` 已从 capture policy 迁出，由 AIO direct TLV helper 显式捕获 pointer array 和前两个 `iocb`。
