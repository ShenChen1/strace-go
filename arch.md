...
- `io_getevents/io_pgetevents` 已迁移为 `payloads`；AIO event 数组长度由 `count_from_ret` 描述，`dynamicSizeStr` 不再保留默认 enter/exit fallback。`io_submit` 已从 capture policy 迁出，由 AIO direct TLV helper 显式捕获 pointer array 和前两个 `iocb`。
