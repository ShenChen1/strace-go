...
- JSON/debug `payload_sections` 不再输出 fixed-window `offset` 字段；外部测试 oracle 只看 kind/direction/arg/user_ptr/user_len/copied_len/probe_ret/data，避免把旧窗口布局固化成机器输出契约。
