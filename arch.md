...
- 新增 `--audit-resolution` 最终 provenance 审计，按 syscall ID 稳定输出实际采用的 metadata source、决策 reason、参数名和类型；reason 会区分 `btf_exact_arity`、`tracepoint_exact_arity`、`no_kernel_metadata` 以及 `btf_arity_mismatch;tracepoint_arity_mismatch` 等拒绝原因。当前主机实测来源仍为 `btf=94`、`tracepoint=250`、`semantic_override=18`、`fallback_override=3`、`dummy=19`，其中 `quotactl`、`preadv`、`pwritev` 的 fallback 是 tracepoint arity mismatch 后的显式决策。这使 fallback 使用量与 override 声明量及内核 ABI 差异分离可见，不能再只看 override audit 推断最终生成表来源。
...
- `fallbackOverrides` 当前仍保留 72 条跨内核声明，但本机最终 resolution 只实际使用 3 条（`quotactl`、`preadv`、`pwritev`）；后续应依据多内核 provenance audit 逐项收缩，而不是根据单机 tracepoint 结果直接删除。
