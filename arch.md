...
- BPF 侧仍保留 `struct bpf_event` / `str_arg` fixed window 作为多条 capture path 的承载结构；目前只有 `getpid/close` 这类 scalar-only syscall 开始绕开旧 carrier，尚未彻底切换为 header + TLV/section-first 的可变长事件协议。
