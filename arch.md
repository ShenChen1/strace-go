...
- 新增 `--audit-resolution` 最终 provenance 审计，按 syscall ID 稳定输出实际采用的 metadata source、参数名和类型；当前主机实测来源为 `btf=94`、`tracepoint=250`、`semantic_override=18`、`fallback_override=3`、`dummy=19`。这使 fallback 使用量与 override 声明量分离可见，不能再只看 override audit 推断最终生成表来源。
