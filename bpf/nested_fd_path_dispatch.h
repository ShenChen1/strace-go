#ifndef STRACE_GO_NESTED_FD_PATH_DISPATCH_H
#define STRACE_GO_NESTED_FD_PATH_DISPATCH_H

/* Tail-call fragments capture one nested select/poll FD path per verifier unit. */
#ifndef STRACE_GO_CORE_ONLY

#if defined(STRACE_GO_ENTER_CONTROL)

struct nested_fd_path_fragment_context {
    struct pending_syscall *pending;
    struct fd_path_scratch *scratch;
};

static __always_inline int load_nested_fd_path_fragment_context(
    struct trace_event_raw_sys_enter *ctx,
    struct nested_fd_path_fragment_context *fragment)
{
    u32 sys_id = (u32)ctx->id;
    if (sys_id != SYS_SELECT && !is_poll_direct_syscall(sys_id)) {
        return 0;
    }
    u32 cfg_key = 0;
...
int enter_nested_fd_path0(struct trace_event_raw_sys_enter *ctx) {
    struct nested_fd_path_fragment_context fragment = {};
    if (!load_nested_fd_path_fragment_context(ctx, &fragment)) {
        return 0;
    }
    ...
    if (fragment.scratch->nested_fd_count > 1) {
        bpf_tail_call(ctx, &enter_progs, ENTER_PROG_NESTED_FD_PATH1);
    }
    return 0;
}

SEC("tracepoint/raw_syscalls/sys_enter")
int enter_nested_fd_path1(struct trace_event_raw_sys_enter *ctx) {
    struct nested_fd_path_fragment_context fragment = {};
    if (!load_nested_fd_path_fragment_context(ctx, &fragment) ||
    ...
    if (fragment.scratch->nested_fd_count > 2) {
        bpf_tail_call(ctx, &enter_progs, ENTER_PROG_NESTED_FD_PATH2);
    }
    return 0;
}

SEC("tracepoint/raw_syscalls/sys_enter")
int enter_nested_fd_path2(struct trace_event_raw_sys_enter *ctx) {
    struct nested_fd_path_fragment_context fragment = {};
    if (!load_nested_fd_path_fragment_context(ctx, &fragment) ||
    ...
    if (fragment.scratch->nested_fd_count > 3) {
        bpf_tail_call(ctx, &enter_progs, ENTER_PROG_NESTED_FD_PATH3);
    }
    return 0;
}

SEC("tracepoint/raw_syscalls/sys_enter")
int enter_nested_fd_path3(struct trace_event_raw_sys_enter *ctx) {
    struct nested_fd_path_fragment_context fragment = {};
    if (!load_nested_fd_path_fragment_context(ctx, &fragment) ||
