    ...
    struct pending_syscall *p)
{
    ...
    u32 copied_len = FD_ARRAY_DIRECT_SIZE;
    s32 probe_ret = 0;
    u32 data_offset = payload_offset + PAYLOAD_TLV_HEADER_SIZE;
    void *payload_data = bpf_dynptr_data(ptr, data_offset, FD_ARRAY_DIRECT_SIZE);
    if (!payload_data) {
        record_ringbuf_copy_fail();
        probe_ret = -1;
        copied_len = 0;
    } else {
        long err = bpf_probe_read_user(payload_data, FD_ARRAY_DIRECT_SIZE, (void *)user_ptr);
        if (err < 0) {
            probe_ret = err;
            copied_len = 0;
        }
...
static __always_inline void emit_fd_array_exit_event_v2_direct(
    ...
    u64 duration)
{
    u32 payload_capacity = PAYLOAD_TLV_HEADER_SIZE + FD_ARRAY_DIRECT_SIZE;
    ...
    u32 payload_size = 0;
    if (ret_value == 0) {
        payload_size = capture_fd_array_tlv_direct(&ptr, payload_offset, p);
    }
    if (payload_size > 0) {
