#ifndef STRACE_GO_SYSCALL_MMSG_EXIT_DIRECT_EVENT_V2_H
#define STRACE_GO_SYSCALL_MMSG_EXIT_DIRECT_EVENT_V2_H
...
static __always_inline void emit_recvmmsg_base2_exit_fragment_event_v2_direct(
    struct pending_syscall *p,
    s64 ret_value,
    u64 duration)
{
    ...
    u16 flags = EVENT_FLAG_EXIT_FRAGMENT;
    u32 payload_size = capture_recvmmsg_base2_exit_payloads_tlv_direct(&ptr, payload_offset, p, &flags);
    if (payload_size > 0) {
        flags |= EVENT_FLAG_PAYLOAD_TLV;
    }
...
static __always_inline void emit_recvmmsg_base3_exit_fragment_event_v2_direct(
    struct pending_syscall *p,
    s64 ret_value,
    u64 duration)
{
    ...
    u16 flags = EVENT_FLAG_EXIT_FRAGMENT;
    u32 payload_size = capture_recvmmsg_base3_exit_payloads_tlv_direct(&ptr, payload_offset, p, &flags);
    if (payload_size > 0) {
        flags |= EVENT_FLAG_PAYLOAD_TLV;
    }
