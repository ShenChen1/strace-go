...
{
    return is_direct_syscall(sys_id) || is_time_struct_direct_syscall(sys_id);
}
    ...
    struct pending_syscall *p)
{
    ...
    u32 copied_len = TIME_DIRECT_TIMESPEC_SIZE;
    s32 probe_ret = 0;
    u32 data_offset = payload_offset + PAYLOAD_TLV_HEADER_SIZE;
    void *payload_data = bpf_dynptr_data(ptr, data_offset, TIME_DIRECT_TIMESPEC_SIZE);
    if (!payload_data) {
        record_ringbuf_copy_fail();
        probe_ret = -1;
        copied_len = 0;
    } else {
        long err = bpf_probe_read_user(payload_data, TIME_DIRECT_TIMESPEC_SIZE, (void *)user_ptr);
        if (err < 0) {
            probe_ret = err;
            copied_len = 0;
        }
    ...
    u64 duration)
{
    ...
    u32 payload_size = 0;
    if (ret_value >= 0) {
        payload_size = capture_time_struct_tlv_direct(&ptr, payload_offset, p);
    }
    if (payload_size > 0) {
