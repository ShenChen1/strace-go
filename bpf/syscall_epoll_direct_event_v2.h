...
static __always_inline void emit_epoll_ctl_enter_event_v2_direct(
    ...
    u64 ts_ns)
{
    u32 payload_capacity = PAYLOAD_TLV_HEADER_SIZE + EPOLL_DIRECT_EVENT_SIZE;
    ...
    u32 payload_size = capture_epoll_ctl_event_tlv_direct(&ptr, payload_offset, ctx);
    if (payload_size > 0) {
...
static __always_inline void emit_epoll_pwait2_enter_event_v2_direct(
    ...
    u64 ts_ns)
{
    u32 payload_capacity = PAYLOAD_TLV_HEADER_SIZE + EPOLL_DIRECT_TIMEOUT_SIZE;
    u32 body_offset = EVENT_V2_HEADER_LEN;
    u32 payload_offset = EVENT_V2_HEADER_LEN + EVENT_V2_ENTER_BODY_LEN;
    u32 out_size = payload_offset + payload_capacity;
    struct bpf_dynptr ptr;
    ...
    u16 flags = EVENT_FLAG_GENERIC_ENTER;
    u32 payload_size = capture_epoll_timeout_tlv_direct(&ptr, payload_offset, ctx);
    if (payload_size > 0) {
        flags |= EVENT_FLAG_PAYLOAD_TLV;
    }
